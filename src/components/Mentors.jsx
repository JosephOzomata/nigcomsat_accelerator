...
export default function MentorSection({ limit = null, showIntro = false }) {
  const { data: rawMentors, loading } = useFirestoreCollection('mentors');
  const [activeCohort, setActiveCohort] = useState(null);

  const grouped = useMemo(() => {
    const sorted = [...rawMentors].sort((a, b) => (a.order ?? 999) - (b.order ?? 999));
    const map = new Map();
    sorted.forEach((m) => {
  ...
  useEffect(() => {
    if (cohorts.length > 0 && !activeCohort) setActiveCohort(cohorts[0]);
    if (activeCohort && !cohorts.includes(activeCohort)) setActiveCohort(cohorts[0] || null);
    ...
    return (
      <section className="bg-gray-50 py-16 px-4 sm:px-6 lg:px-8">
        <div className="max-w-6xl mx-auto text-center">
          <h2 className="text-4xl font-extrabold text-gray-900 tracking-tight sm:text-5xl">Our Mentors</h2>
  ...
  const activeMentors = activeCohort ? grouped.get(activeCohort) : [];
  const displayedMentors = limit ? activeMentors.slice(0, limit) : activeMentors;
      ...
      <div className="w-full mx-auto">
        ...
        {showIntro && (
          ...
          >
            ...
            <h1 className="text-4xl mt-13 md:text-5xl font-bold text-gray-900 tracking-tight mb-4">
              Our Mentors
            </h1>
            <p className="text-base md:text-lg text-gray-600 leading-relaxed">
              The NIGCOMSAT Accelerator is powered by experienced operators,
              founders, engineers, and policy leaders who dedicate their time
              to guiding each cohort. Our mentors come from across the space
              industry, private sector, and academia — offering hands-on
              support in product, business, and technical development.
            </p>
          </motion.div>
        )}
            ...
            {cohorts.map((cohort) => (
              <button
                ...
                className={`px-6 py-2.5 text-sm font-semibold rounded-lg transition-all duration-200 relative z-10 ${
                  activeCohort === cohort ? 'text-black shadow-sm' : 'text-gray-600 hover:text-gray-900'
                }`}
              >
                {cohort}
                {activeCohort === cohort && (
                  ...
                  >
                    ...
                    <div className="p-6 flex-1 flex flex-col justify-between">
                      <div>
                        ...
                        <p className="mt-3 text-sm text-gray-500 leading-relaxed">{mentor.bio}</p>
                      </div>
