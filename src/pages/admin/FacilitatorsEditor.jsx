...
const FacilitatorsEditor = () => {
  ...
  useEffect(() => {
    const unsubItems = subscribeCollection('facilitators', setItems);
  ...
  const save = async () => {
    ...
    try {
      ...
      if (id) await updateItem('facilitators', id, payload);
      else await createItem('facilitators', payload);
  ...
  const requestDelete = (p) => setConfirmState({
    ...
    confirmLabel: 'Delete',
    onConfirm: async () => { await deleteItem('facilitators', p.id); toast.success('Deleted'); },
  });
    ...
    <>
      ...
      <div>
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Facilitators</h1>
            <p className="text-sm text-gray-500 mt-1">Assigned to a cohort. Shown on /facilitators and cohort pages.</p>
          </div>
        ...
        <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
          {items.length === 0 ? (
            <div className="p-8 text-center text-gray-500 text-sm">No facilitators yet.</div>
          ) : (
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ opacity: 0, y: 20, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }}
              ...
              <div className="flex items-center justify-between p-5 border-b border-gray-200">
                <h2 className="font-bold text-gray-900">{editing.id ? 'Edit Facilitator' : 'New Facilitator'}</h2>
              ...
              <div className="p-5 space-y-4 max-h-[70vh] overflow-y-auto">
                ...
                <Field label="Role" hint="e.g. Lead Facilitator">
  <Input value={editing.role || ''} onChange={set('role')} />
</Field>
...
export default FacilitatorsEditor;